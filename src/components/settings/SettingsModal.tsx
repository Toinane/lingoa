...
import { useSettingsStore, THEME_REGISTRY, type Language } from "../../stores/settingsStore";
    ...
    >
      <div className="w-full max-w-md bg-app-surface border border-app-border rounded-xl shadow-2xl overflow-hidden">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-app-border">
          <h2 className="text-app-text font-semibold text-sm">{t.settings.title}</h2>
        ...
        <div className="overflow-y-auto max-h-[calc(100vh-8rem)]">
          ...
          <section className="px-5 py-4 border-b border-app-border">
            <h3 className="text-app-muted text-xs uppercase tracking-wider mb-3">{t.settings.account}</h3>

            {currentUser ? (
              <>
                <div className="flex items-center justify-between mb-3">
                  <div>
                    <div className="text-app-text text-sm font-medium">@{currentUser}</div>
                    <div className="text-app-muted text-xs">{t.settings.githubAccount}</div>
                  </div>
                  <div className="flex gap-2">
                    <button
                      onClick={() => { setShowTokenInput((v) => !v); setTokenError(null); }}
                ...
                {showTokenInput && (
                  <div className="space-y-2">
                    ...
                    />
                    {tokenError && <p className="text-key-red text-xs">{tokenError}</p>}
                    <div className="flex gap-2">
                      <button
              ...
              <div className="space-y-2">
                <p className="text-app-muted text-xs mb-3">{t.auth.description}</p>
                <input
                  type="password"
                  value={tokenValue}
                  onChange={(e) => setTokenValue(e.target.value)}
          ...
          {/* Appearance */}
          <section className="px-5 py-4 border-b border-app-border">
            <h3 className="text-app-muted text-xs uppercase tracking-wider mb-3">{t.settings.appearance}</h3>

            <div className="mb-4">
              <p className="text-app-text text-xs font-medium mb-2">{t.settings.theme}</p>
            ...
            <div>
              <p className="text-app-text text-xs font-medium mb-2">{t.settings.language}</p>
              <select
                value={language}
          ...
          <section className="px-5 py-4">
            <h3 className="text-app-muted text-xs uppercase tracking-wider mb-3">{t.settings.editor}</h3>
